...
public class StreamTest4Reduce {
    public static void main(String[] args) {
        ...
        Optional<Double> somaSalario = streamPessoa.filter(pessoa -> pessoa.getSalario() > 4000)
                .map(Pessoa::getSalario)
                .reduce(Double::sum);
        System.out.println(somaSalario.get());

        Double somaDouble = Pessoa.bancoDePessoas().stream().filter(pessoa -> pessoa.getSalario() > 4000)
